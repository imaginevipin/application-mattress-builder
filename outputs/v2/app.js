...
function renderTopMain() {
  ...
  stack.innerHTML = state.topLayers.map((layer, idx) => {
    ...
    const linkButtons = [];
    if (!layer.quiltingAdded) linkButtons.push(`<button class="add-link" data-top-add="quilting" data-idx="${idx}">+ Top Quilting</button>`);
    if (!layer.gussetAdded) linkButtons.push(`<button class="add-link" data-top-add="gusset" data-idx="${idx}">+ Gusset Quilting</button>`);
    if (!layer.tuftsAdded) linkButtons.push(`<button class="add-link" data-top-add="tufts" data-idx="${idx}">+ Tufts</button>`);
...
function renderWallMain() {
  ...
  if (state.wallQuiltingAdded) {
    const addTuftsHtml = !state.wallTuftsAdded
      ? `<div class="add-links--in-card"><button class="add-link" id="wallAddTufts">+ Tufts</button></div>`
      : '';
    cards += makeLayerCard({
  ...
  if (state.wallTuftsAdded) {
    const addQuiltingHtml = !state.wallQuiltingAdded
      ? `<div class="add-links--in-card"><button class="add-link" id="wallAddQuilting">+ Wall Quilting</button></div>`
      : '';
    cards += makeLayerCard({
      id: 'wallTufts', thumbCss: 'ptn-buttons',
      label: 'Tufts', value: getWallTuftName(),
      showDelete: true, expanded: state.wallTuftsExpanded,
      bodyHtml: tuftsBodyHTML('wallTufts', state) + addQuiltingHtml,
    });
  }
...
function initAssistant() {
  ...
  function switchVersion(v) {
    ...
    // Hide Top/Wall/Bottom tab strip in v1 and v3 — nav items handle switching; only v2 needs the strip
    const externalTabStrip = document.getElementById('externalTabStrip');
    if (externalTabStrip) externalTabStrip.hidden = (v !== 2);
    ...
    if (v !== 3) closeV3Panel();
    myRequestsBtn.textContent = v === 2 ? 'My Requests' : v === 3 ? 'Request' : 'Request Props';
