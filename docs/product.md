...
- Internal mode rebuilt closer to Live screenshots — split into `Layers` and `Layouts` sections; `Layers` now shows a compact layer stack with height sliders, inline management icons, and an `Add new layer` footer CTA; `Layouts` now holds `Exploded`, `Inner Build`, and `Cutaway View` controls with screenshot-matched grouping
- Images panel: now live — `Previews` and `Renders` tabs, empty states, generated image cards, render queue records, and full-image viewer overlay
- Create Image flow: now live — top-right button opens a dark modal with Preview / High Resolution modes, saved camera selection, resolution presets, custom width/height, filename + JPG/PNG format, transparent background and floor-shadow toggles; created outputs flow into Images
- Add to Library flow: now live — top-right button and Style Properties `Save in library` button open a save modal with live preview capture, mattress name field, availability counter, and save action
- Cameras panel: now live — Add Current View, Import Camera Presets, search, selectable saved cameras, and viewport camera recall
...
- Top-right action buttons: `Create Image` and `Add to library` are now implemented; `Request Prop` still needs product definition
- All external panels (Size, Height, Top, Wall, Tape, Label, Handle, Bottom) are fully implemented
- Internal mode now has live `Layers` + `Layouts` panels, and the Three.js viewport supports screenshot-aligned internal rendering states for exploded stack views, inner-build presentation, and cutaway reveal controls
- Interactive 3D viewport: hovering over any external mattress part (top quilting surface, side walls, tape border, body/shell) shows an orange emissive highlight and a pointer cursor; clicking jumps directly to the matching sidebar panel (Top, Wall, Tape, Bottom); orbit drag is unaffected — panel switch only fires on a true click; inactive in Internal mode
...
- Internal layer state now carries a configurable `layers` array plus screenshot-driven layout state (`layersExploded`, `layersExplodedGap`, `layersInnerBuild`, `layersCutaway`); viewport.js exposes sync hooks for mode/layer/internal-view updates
